...
DynamicWindowApproach::DynamicWindowApproach(ros::NodeHandle& nh) : nh_(nh), tf_buffer_(), tf_listener_(tf_buffer_)
{
  ...
  timer_listen_goal_ =
      nh_.createTimer(ros::Duration(1 / PUBLISH_HZ_), &DynamicWindowApproach::timerListenGoalCallback, this);
...
std::vector<geometry_msgs::Point::Ptr>
DynamicWindowApproach::createObstaclePoints(const geometry_msgs::Pose2D::Ptr& pose)
{
  ...
  for (size_t i = 0; i < static_cast<size_t>(laser_scan_->ranges.size()); ++i)
  {
    ...
    // obstacle_points[i]->x = pose->x + X_LIDER_ * std::cos(pose->theta) + laser_scan_->ranges[i] *
    // std::cos(laser_angle); obstacle_points[i]->y = pose->y + X_LIDER_ * std::sin(pose->theta) +
    // laser_scan_->ranges[i] * std::sin(laser_angle);
  }
  return obstacle_points;
}

geometry_msgs::Twist::Ptr DynamicWindowApproach::selectBestTwist(const geometry_msgs::Pose2D::Ptr& pose,
                                                                 const geometry_msgs::Twist::Ptr& twist,
                                                                 const Window& window)
{
...
std::vector<geometry_msgs::Pose2D::Ptr> DynamicWindowApproach::predictPath(const geometry_msgs::Pose2D::Ptr& pose,
                                                                           const geometry_msgs::Twist::Ptr& next_twist)
{
...
double DynamicWindowApproach::evaluateTargetHeading(const geometry_msgs::Pose2D::Ptr& next_pose)
{
  double angle_to_goal =
      std::atan2(goal_->transform.translation.y - next_pose->y, goal_->transform.translation.x - next_pose->x);
  double delta_theta = angle_to_goal - next_pose->theta;
  double evaluation_value = std::fabs(normalizeAngle(delta_theta)) / M_PI;
  return evaluation_value;
}

double DynamicWindowApproach::evaluateClearance(const geometry_msgs::Pose2D::Ptr& pose,
                                                const std::vector<geometry_msgs::Pose2D::Ptr>& path)
{
...
void DynamicWindowApproach::goalJudgment(const geometry_msgs::Pose2D::Ptr& pose)
{
  ...
  if (goal_x - GOAL_THRESHOLD_ < pose->x && pose->x < goal_x + GOAL_THRESHOLD_ && goal_y - GOAL_THRESHOLD_ < pose->y &&
      pose->y < goal_y + GOAL_THRESHOLD_)
  {
    reach_goal_ = true;
  }
}
